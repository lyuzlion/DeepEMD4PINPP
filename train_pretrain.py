...
parser.add_argument('-dataset', type=str, default='pinpp', choices=['miniimagenet', 'cub','tieredimagenet','fc100','tieredimagenet_yao','cifar_fs', 'pinpp'])
...
args.save_path = 'pre_train/%s/%d-%.4f-%d-%.2f/' % (dataset_name, args.bs, args.lr, args.step_size, args.gamma)
...
ensure_path(args.save_path) # 不存在就新建

args.dir = 'pretrained_model/miniimagenet/max_acc.pth'

Dataset=set_up_datasets(args)
trainset = Dataset('train', args)
train_loader = DataLoader(dataset=trainset, batch_size=args.bs, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)

valset = Dataset(args.set, args)
val_sampler = CategoriesSampler(valset.label, args.num_episode, args.way, args.shot + args.query) # 100, 5, 1+9
val_loader = DataLoader(dataset=valset, batch_sampler=val_sampler, num_workers=8, pin_memory=True)
if not args.random_val_task:
    print('fix val set for all epochs')
    val_loader = [x for x in val_loader]
# print(len(val_loader)) # 100
# print((val_loader[0])) # 2, images, labels
# print(len(val_loader[0][0])) # 50, 5way * (1shot + 9query)
