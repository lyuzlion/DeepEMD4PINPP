...
PRETRAIN_DIR='./checkpoint/pre_train'
# DATA_DIR='/home/zhangchi/dataset'
DATA_DIR='/home/liuzilong/data2/datasets'

parser = argparse.ArgumentParser()
#about dataset and training
parser.add_argument('-dataset', type=str, default='pinpp', choices=['pinpp'])
...
parser.add_argument('-val_frequency',type=int,default=25)
...
parser.add_argument('-val_episode', type=int, default=25, help='number of validation episode')
parser.add_argument('-test_episode', type=int, default=25, help='number of testing episodes after training')
# about model
...
# model
args.pretrain_dir=osp.join(args.pretrain_dir,'%s/128-0.0400-30-0.20/max_acc.pth'%(args.dataset))
model = DeepEMD(args)
...
# Test Phase
trlog = torch.load(osp.join(args.save_path, 'trlog'), weights_only=False)
...
model.load_state_dict(torch.load(osp.join(args.save_path, 'max_acc' + '.pth'), weights_only=False)['params'])
model.eval()
